...
# Given image of uniform texture turn it into PBR material (new version small change in probabilities)
    ...
    # generate map for each property ----------------------------------
        for ky in ['Height', 'Roughness', 'Metallic']:
            ...
            # ----------------Generate Patterned map from image channel---------------------------------------------------------
                ...
                r = np.random.rand()
                if r<0.2 or  (ky == 'Height' and r<0.4):
                      mp = im.mean(2)
                elif r<0.64:
            ...
            #............Blure augment.............................
                if ky == 'Roughness':
                    use_negative =  np.random.rand() < 0.6
                else:
                    use_negative = np.random.rand() < 0.3
                if use_negative: # negative of map
                    mp = 255 - mp
                if np.random.rand() < 0.11: # blurring
